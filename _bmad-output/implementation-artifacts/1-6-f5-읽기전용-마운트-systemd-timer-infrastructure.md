...
### Review Findings (post-facto CR, 2026-04-23)

본 스토리는 PR #13 → PR #14 (review-flip) 흐름에서 정식 `/bmad-code-review` 단계가 누락된 채 `done` 으로 플립됨. 2026-04-23 post-facto CR (3-layer adversarial: Blind Hunter + Edge Case Hunter + Acceptance Auditor) 실행 결과 다음 발견사항을 기록함. 코드는 이미 master 에 landing 되어 있으므로 HIGH 건은 follow-up 스토리(1.6.1 또는 Story 1.9 observability hardening) 에서 patch, 나머지는 `deferred-work.md` 로 승계.

**Patch 후보 (2026-04-23 Option 0 batch-apply — 모두 inline 수정 완료, follow-up PR 대기)**:

- [x] **[Review][Patch][HIGH] `SuccessExitStatus=0 1` 가 holiday skip 의도를 깨뜨림** → `ExecStartPre` 제거 + `ExecCondition=check_trading_day.py` 전환 + `SuccessExitStatus=0 1` 삭제. ExecCondition exit 1-254 는 systemd-native "silent skip (no failure)" 이므로 holiday 에 ExecStart 가 실행되지 않음. CLI PARTIAL exit 1 도 이제 unambiguous 실패로 `Restart=on-failure` 정상 동작. Trade-off: ExecCondition skip 시 ExecStopPost 미발화 → holiday 에 메트릭 미갱신 (state 불변이므로 정확함, Story 1.9 alert rule 이 weekend gap 허용 필요). [`infra/systemd/athena-readonly-mount-lock.service`, `athena-readonly-mount-unlock.service`, `tests/integration/test_readonly_mount_units.py::test_services_do_not_use_success_exit_status_whitelist`]
- [x] **[Review][Patch][HIGH] `OverrideAttemptEvent` + `ReadonlyMountController._validate_protected_paths` 가 `..` traversal 수용** → 양쪽 모두 `if ".." in path.parts: raise ValueError` 가드 추가. Regression 테스트 `test_dotdot_traversal_path_rejected` (readonly_mount) + `test_target_path_with_dotdot_rejected` (override_event) 신규. [`packages/athena-alpha-defense/athena/alpha_defense/f5/readonly_mount.py:_validate_protected_paths`, `override_event.py:__post_init__`]
- [x] **[Review][Patch][HIGH] `check_trading_day.py:94` 의 `date.today()` 가 system-local TZ 에 의존** → `from zoneinfo import ZoneInfo; _KST = ZoneInfo("Asia/Seoul"); today = datetime.now(_KST).date()`. install.sh 의 Asia/Seoul warn 은 유지하되, Python 레벨에서 방어. 기존 `--as-of` 테스트는 결정론적 date 객체이므로 영향 없음. [`scripts/check_trading_day.py:19-27, 95`]
- [x] **[Review][Patch][MEDIUM] install.sh DRY_RUN 이 `visudo -cf` 를 스킵** → `install_sudoers` 에서 DRY_RUN/real 공통 fail-fast 경로로 `visudo -cf` 선행 실행. dry-run 은 `visudo -cf $src -> OK (validated)` 로그 찍고 real install 부분만 스킵. [`infra/systemd/athena-readonly-mount.install.sh:install_sudoers`]
- [x] **[Review][Patch][MEDIUM] install.sh `run()` helper 는 `eval "$@"` + 호출자 0명 (dead code)** → 함수 삭제 + post-CR 주석. [`infra/systemd/athena-readonly-mount.install.sh:dry/run`]
- [x] **[Review][Patch][MEDIUM] `_run_sudo_chattr` 에 `sudo -n` 누락** → `["sudo", "-n", "/usr/sbin/chattr", *args]`. 테스트의 `_ensure_sudo_chattr_nopasswd` probe 와 대칭. [`packages/athena-alpha-defense/athena/alpha_defense/f5/readonly_mount.py:_run_sudo_chattr`]
- [x] **[Review][Patch][MEDIUM] `ReadonlyMountController._transition` 의 post-loop `status()` 호출이 예외 unhandled** → `status()` 자체를 per-path try/except 로 감싸 probe failure → 비immutable 으로 aggregation. 신규 `test_status_is_resilient_to_probe_exceptions` 추가. [`packages/athena-alpha-defense/athena/alpha_defense/f5/readonly_mount.py:status`]
- [x] **[Review][Patch][MEDIUM] install.sh `seed_policy_dir` 가 `policy.toml` 에 대해선 fallback 없이 `install` 실행** → `flag_registry.toml` 의 empty-stub 분기 패턴을 `policy.toml` 에도 대칭 적용. [`infra/systemd/athena-readonly-mount.install.sh:seed_policy_dir`]
- [x] **[Review][Patch][LOW] `DryRunChattrExecutor` docstring claim 이 실제 동작과 불일치** → docstring 을 "single CLI invocation lifetime only" 로 정정, 기존 테스트의 관찰 사실과 일치하도록 명시. [`packages/athena-alpha-defense/athena/alpha_defense/f5/readonly_mount.py:DryRunChattrExecutor`]
- [x] **[Review][Patch][MINOR] 본 파일 File List 헤더 "Created (17 신규)" 와 실제 23 bullet 불일치** → "23 신규" 로 정정. [`_bmad-output/implementation-artifacts/1-6-f5-*.md:File List`]

**Deferred (code review of 1-6-f5-* 하위 `## Deferred from: code review` 로 이관)**:

- [x] [Review][Defer] `_read_prev_timestamp` concurrent-emit race + parse-error silent demotion — `metrics.py:57-74,156-165`. 현 V1.0 scope 에서 lock+unlock 동시 발화 확률 낮음, Story 1.9 observability 에서 advisory lock 도입 검토.
- [x] [Review][Defer] `_flags_contain_immutable` lsattr warning-preamble 파싱 견고성 — `readonly_mount.py:112-122`. 9p/NFS 환경은 V1.0 scope 아님 (WSL2 ext4 전용).
- [x] [Review][Defer] `ConditionPathExists=` 실패 시 silent no-op — `lock.service:13-14` + `unlock.service:8-9`. Story 1.9 의 `athena_readonly_mount_last_transition` 노화 알럿이 간접적으로 cover; 명시적 "policy dir health" 메트릭은 별도 스토리.
- [x] [Review][Defer] `test_end_to_end_market_cycle` 의 `journalctl -u` 검증 누락 — `tests/integration/test_chattr_e2e.py:183-213`. 현 테스트는 controller 직접 호출이므로 systemd journal 이 발생하지 않아 spec step 이 사실상 infeasible. Story 3.5 의 `systemctl start ...` 경로 통합 테스트에서 합치적으로 cover 예정.
- [x] [Review][Defer] `test_install_sh_is_idempotent_on_dry_run_repeats` 의 "skip ... already installed" 메시지 부재 검증 — `tests/integration/test_readonly_mount_units.py:237-255`. 실모드 설치가 sudo 필요 → CI self-hosted runner 의 non-interactive sudo 가능성 확보 후 (Story 1.7 host setup) 추가.
- [x] [Review][Defer] `holidays.country_holidays("KR", years=today.year)` 가 현재 연도만 load + `/etc/athena/extra_closed_days.txt` 기본 경로를 installer 가 생성 안 함 — `check_trading_day.py:97`, `install.sh`. Story 1.10 host-setup 단계에서 `install.sh` 가 빈 파일 + 헤더 주석 touch 하도록 확장.
- [x] [Review][Defer] `LockTransition.per_file_results` dict key 가 `PurePosixPath` → Story 3.1 `json.dumps(asdict(...))` 시 TypeError — `readonly_mount.py:193`. Story 3.1 이 canonical JSON 으로 직렬화할 때 본 dataclass 를 consume, 경계에서 `str(k): v` 변환 필요 — Story 3.1 스펙 작성 시 명시.

**Dismissed (9건)** — `_LooseValidationController` test subclass (설계 의도, docstring 명시), `khuk0` 하드코딩 (이미 deferred to Story 1.7), `SubprocessChattrExecutor` 플랫폼 가드 부재 (Dev Agent Record Architecture Patterns 에 명시 drift), `test_git_revert_blocked_during_lock` 이 `cp -f` 사용 (EPERM 프리미티브 동일, docstring 명시 단순화), `runtime_checkable=False` 생략 (Protocol default=False), install.sh `install` 의 `-o root -g root` 비대칭 (cosmetic), File List 헤더 stale count 는 위 Patch 건으로 승격, working-tree 의 `config/flag_registry.toml` untracked (commit 경계 밖), `@dataclass(frozen=True)` + mutable dict (spec 그대로 구현).
...
| 2026-04-23 | 0.4.0 | **Post-facto code review** — PR #13/#14 흐름에서 정식 `/bmad-code-review` 단계가 생략된 채 done 으로 플립된 것을 보완. 3-layer adversarial (Blind Hunter + Edge Case Hunter + Acceptance Auditor) 실행 → 26건 발견 중 중복 제거 후 core 15건 (HIGH 3 / MEDIUM 8 / LOW/MINOR 4). Triage: patch 9 (HIGH 3 — SuccessExitStatus+exit 1 collision, `..` path traversal, local-TZ date.today ; MEDIUM 5 — DRY_RUN visudo skip, install.sh eval dead code, sudo -n missing, _transition post-status unhandled, policy.toml seed fallback ; LOW/MINOR 2 — DryRun docstring mismatch, File List header count) / defer 7 (`_read_prev_timestamp` race, lsattr warning-preamble, ConditionPathExists silent no-op, journalctl 검증 누락, install.sh idempotency skip-message 검증, holidays year boundary + extra_closed default 파일 미생성, LockTransition dict key JSON 경계 변환) / dismiss 9 (설계 의도 / 이미 deferred / 문서화된 drift / cosmetic). 코드는 이미 master 에 landing 됐으므로 patch 건들은 follow-up PR (후속 bugfix 스토리) 에서 처리, defer 는 `deferred-work.md` 에 기록. 본 스토리의 Status=done 은 유지 (코드 재변경 없음). | Amelia via bmad-code-review |
