...
Code review run 2026-04-22 (Blind Hunter + Edge Case Hunter + Acceptance Auditor parallel layers). Scope: diff `2f95bb6..HEAD` = Story 1.2 code only (8 files, 919 lines). Acceptance Auditor verdict: **all 5 ACs satisfied within scope; no Critical/Major AC violations.** Adversarial layers surfaced hardening gaps listed below. Initial triage raised 2 decision-needed items; on re-review both resolve to dismissal (AC-3's explicit fail-fast invariant and Athena's Kill-Switch philosophy align with current behaviour); one was promoted to a docstring patch (P14).

**Patch (14) — all applied 2026-04-22:**

- [x] [Review][Patch] AST shell-call guard bypassable via reflection — add `getattr`, `__import__`, `eval`, `exec`, `compile` to `FORBIDDEN_BARE_CALLS`; add `importlib` to `FORBIDDEN_TOPLEVEL_MODULES`. Current guard misses `getattr(os, "system")(...)` and `__import__("subprocess").run(...)`. [packages/athena-core/tests/test_keyring_client_no_shell.py:25,68]
- [x] [Review][Patch] `.env/` as a directory bypasses top-level guard — `_is_dotenv_file` requires `is_file()`; a directory literally named `.env/` is not flagged at depth 0. Match by name regardless of file/dir at the top level. [packages/athena-core/athena/core/settings.py:75-78,87-88]
- [x] [Review][Patch] `.ENV` uppercase variant bypass on Windows/WSL case-preserving mounts — `path.name == ".env"` is case-sensitive; `.ENV` passes through while downstream python-dotenv loaders on Windows would still read it. Normalize `path.name.lower()` before comparison. [packages/athena-core/athena/core/settings.py:87-88]
- [x] [Review][Patch] `.env` symlink inconsistency between runtime and regression — runtime `is_file()` follows symlinks (catches valid links, misses broken links); regression walker does `if entry.is_symlink(): continue` (misses all). Align: runtime should also flag broken `.env` symlinks; regression should NOT skip symlinks whose NAME matches `.env*`. [packages/athena-core/athena/core/settings.py:87-88] [tests/regression/test_no_dotenv_files.py:62-63]
- [x] [Review][Patch] Symlinked depth-1 subdirectory enables home-dir traversal — `settings.py:80` uses `subdir.is_dir()` which follows symlinks. A dev convenience link like `ln -s $HOME mydocs` at repo root makes the guard walk `$HOME` and trip on the user's real `~/.env`, killing Athena startup over a workstation artefact unrelated to the repo. Add `and not subdir.is_symlink()` to the subdir guard. [packages/athena-core/athena/core/settings.py:79-84]
- [x] [Review][Patch] `PermissionError`/`OSError` from `iterdir` breaks the AC-3 fixed `SystemExit(".env usage forbidden by NFR-S1: ...")` contract — wrap `root.iterdir()` / `subdir.iterdir()` in try/except matching the regression walker's pattern (`(PermissionError, FileNotFoundError, OSError)`). A stale WSL mount or protected depth-1 dir currently bubbles a raw stack trace. [packages/athena-core/athena/core/settings.py:72-84]
- [x] [Review][Patch] Empty-string secret passes the `is None` check — caller receives `""` and later authentication fails with opaque HTTP 401. Use `if not value: raise MissingSecretError(...)` to collapse `None` and `""` into the same contract. [packages/athena-core/athena/core/keyring_client.py:77-80]
- [x] [Review][Patch] `ATHENA_REPO_ROOT` override lacks validation — whitespace-only value (e.g. `"  "`) is truthy and `Path("  ").resolve()` yields CWD; nonexistent path walks into non-directory. Add `if not override.strip(): override = None`; on non-existent/non-dir after resolve, raise `ValueError` to fail-loud. [packages/athena-core/athena/core/settings.py:34-40]
- [x] [Review][Patch] `del fake_store` is a misleading no-op — `del` only drops the local binding; the closure inside `fake_get` still references the original dict. Test still exercises the right branch but the code reads wrong. Remove the `del` line (store is already empty). [packages/athena-core/tests/test_keyring_client.py:60-66]
- [x] [Review][Patch] `get_settings` LRU cache and `ATHENA_*` env pollution leak across tests — `test_get_settings_is_singleton` calls `cache_clear()` on entry but nothing clears on teardown; `test_settings_defaults` breaks if shell has `ATHENA_ENVIRONMENT`/`ATHENA_APP_LOG_LEVEL` set. Add an autouse conftest fixture that `delenv("ATHENA_...", raising=False)` and `get_settings.cache_clear()` per-test. [packages/athena-core/tests/test_settings.py:25-29,122-126]
- [x] [Review][Patch] Test name `test_dotenv_guard_ignores_env_example` contradicts its body — the assertion is `pytest.raises(SystemExit, ...)` (rejects, not ignores). Rename to `test_dotenv_guard_rejects_env_example`. Future `grep` for intent is wrong today. [packages/athena-core/tests/test_settings.py:100-105]
- [x] [Review][Patch] `test_exclude_lists_stay_in_sync` is one-way only despite the name — only `SETTINGS_EXCLUDE_DIRS - EXCLUDE_DIRS` is asserted empty. The docstring justifies asymmetric cache-dir handling, but the name misleads. Rename to `test_regression_excludes_are_superset_of_runtime` (or split into two tests with explicit allow-list for the intended asymmetry). [tests/regression/test_no_dotenv_files.py:85-99]
- [x] [Review][Patch] AST no-shell test has no empty-file regression — `test_keyring_client_file_exists` only asserts existence; if `keyring_client.py` is accidentally truncated to `""`, all four no-shell tests pass trivially (zero AST nodes = zero violations) and give false confidence. Add an assertion that the parsed tree contains `keyring.get_password` and `keyring.set_password` calls. [packages/athena-core/tests/test_keyring_client_no_shell.py:131]
- [x] [Review][Patch] `get_secret` docstring silent on keyring backend failure — raw `keyring.errors.KeyringError` propagates on headless WSL2 without Secret Service. Intentional per Kill-Switch philosophy (infrastructure failure = process death), but undocumented. Add 2-line docstring note: "backend unavailable (e.g. no Secret Service on headless WSL) propagates `keyring.errors.KeyringError` unchanged — this is an infrastructure/setup failure, not a missing secret". [packages/athena-core/athena/core/keyring_client.py:66-80]

**Deferred (pre-existing / cross-cutting):**

- [x] [Review][Defer] Runtime `_EXCLUDE_DIRS` missing `.mypy_cache` / `.ruff_cache` / `.pytest_cache` / `.import_linter_cache` — regression walker already excludes them; runtime depth-1 guard could trip if a tool ever writes `.env` into a cache dir. Deferred: AC-3 enumerates 8 dirs explicitly and cache-dir `.env` is extremely unlikely; widening the list is spec-expansion. Revisit if ever observed. [packages/athena-core/athena/core/settings.py:45-56]
- [x] [Review][Defer] Windows junction point (`mklink /J`) behavior inconsistent with POSIX symlink in regression walker — `pathlib.Path.is_symlink()` returns `True` for junctions in modern Python but behavior has shifted historically; regression walker's blanket `if entry.is_symlink(): continue` skips all junctions, potentially hiding `.env` at a junction target. Deferred: platform quirk requiring cross-platform CI (currently single-host); revisit during Story 1.3 CI hardening. [tests/regression/test_no_dotenv_files.py:62]

**Dismissed as noise (13 — not persisted):** import-time `SystemExit` UX vs. AC-3 explicit fail-fast (spec mandates; depth-1 guard and full-tree regression walker are additive, not overlapping — "nice report" scenario is marginal); keyring backend failure propagates raw `KeyringError` (Kill-Switch philosophy — infrastructure failure ≠ missing secret, and no caller benefits from merging the two failure modes today — YAGNI for a new error class; docstring clarification covered by P14); `parents[4]` hardcoded offset (by-design, `ATHENA_REPO_ROOT` override is the escape hatch per Dev Notes invariant 5); `Literal` value case-sensitivity while env-var name is case-insensitive (standard pydantic); `set_secret` silent overwrite (dev-bootstrap-only per docstring); missing `delete_secret` primitive (out of scope); `StrEnum` py3.11+ dependency (project targets py3.13); no validation of raw-string secret names (intentional — `test_get_secret_accepts_raw_str` enshrines flexibility for bootstrap scripts); depth-2 `.env` undetected by runtime (documented design; regression walker covers); AC-5 firewall `172.16/12` broader than `LocalSubnet` (documented in Debug Log #10 with WSL vEthernet NetworkCategory justification); Task-1/5/6 parent-`[x]` / subtask-`[ ]` pattern (host-setup convention); `S105` scoped to whole `keyring_client.py` (pragmatic; file contains only the enum, documented with a why-comment in pyproject.toml); regression `_EXCLUDE_DIRS` name-based depth-agnostic pruning (gitignore-style convention; collision with user-authored `build/` dir is negligible).
...
- `packages/athena-core/tests/conftest.py` (Review P10 — autouse env + cache isolation)
...
| 2026-04-22 | 1.1.0 | Story 1.2 → done. Code review (3-layer parallel: Blind Hunter + Edge Case Hunter + Acceptance Auditor) surfaced 28 findings: 14 patches applied, 2 deferred to `deferred-work.md`, 2 decision-items re-triaged to dismiss, 10 dismissed as noise. Hardenings: AST reflection-bypass closure (getattr/__import__/eval/exec/compile), `_ensure_no_dotenv_files` case-insensitive + symlink/dir/permission hardening, `ATHENA_REPO_ROOT` validation, empty-string-secret → `MissingSecretError`, `get_secret` backend-failure docstring, autouse conftest env+cache isolation, AST empty-file regression guard, two test-name clarifications. +2 tests (115 → 113 pass / 2 skip). 5-gate green. | Amelia (dev, review) |
| 2026-04-22 | 1.2.0 | Post-facto: WSL2 toolchain gap closed mid-Story-1.3 session. `uv 0.11.7` (astral installer), CPython 3.13.13 (`uv python install`), `gh 2.45.0` (apt), `.venv` reproduced via `uv sync --frozen --group dev` (64 packages), pre-commit hooks installed for pre-commit + commit-msg stages. `uv run pytest -n auto` on Linux: 127 passed / 3 skipped (Windows: 126p/4s; delta = uvloop import test runs on Linux). No code/test change; SSH signing infra from commit `85895b2` unchanged. Retroactively completes the implicit "WSL2 dev environment reproducible" step that Story 1.2 never named explicitly. Story 1.7 (Windows host SSH signing) now optional rather than blocking — WSL2 is the single git-commit origin until Logger PC lands. | Amelia (dev, Story 1.3 session) |
| 2026-04-22 | 1.3.0 | Post-facto AC-4 closure: WSL2 signing SSH public key (`~/.ssh/id_ed25519_athena_sign.pub`) uploaded to GitHub account `ha-nyang-95` as Signing Key. Story 1.2 AC-4 originally targeted "`git log --show-signature` locally works" (covered by `~/.ssh/allowed_signers`), but GitHub-side verification (needed once `required_signatures: true` protection is active) requires the key in the account-level signing slot — which was never populated. Before upload: every WSL2 commit had `"verified": false, reason: "unknown_key"` server-side. After upload (2026-04-22 ≈ 07:50 KST): all commits from the same key re-evaluate as `"verified": true, reason: "valid"` retroactively. Gap surfaced only when Story 1.3 Task 5 activated `required_signatures` and PRs started failing merge with "Commits must have verified signatures". Fix is single web-UI action (`github.com/settings/ssh/new` → type=Signing Key → paste public key). | Amelia (dev, Story 1.3 session) |
