...
**Patch (MAJOR):**
- [x] [Review][Patch] `parse_hour` 범위·포맷 검증 부재 (`now-0`, `now--5`, `YYYY-MM-DD HH` 공백 등 silent no-op 또는 bare traceback) [`scripts/export_parquet_shard.py:parse_hour`]
- [x] [Review][Patch] rsync `--chown=khuk0:khuk0` 미설정 — UID mismatch 시 파일 unreadable → 0 rows silent [`infra/systemd/athena-logger-sync.service:ExecStart`]
- [x] [Review][Patch] rsync `--partial-dir=.rsync-partial` 이 `/data/parquet/` 내부 → hive partition namespace 오염 (rglob 가 미완성 `.parquet` 포착) [`infra/systemd/athena-logger-sync.service:ExecStart`]
- [x] [Review][Patch] `FeatureStore.refresh_views()` 메서드 추가 — long-lived Trading PC 프로세스가 rsync 된 신규 shard 를 재시작 없이 반영 [`packages/athena-feature-store/athena/feature_store/feature_query.py:FeatureStore`]
- [x] [Review][Patch] `attach_parquet_views` TOCTOU race — `has_shards` 체크와 `read_parquet` 사이 rsync 경합 시 view 생성 실패; try/except 재시도 필요 [`parquet_reader.py:attach_parquet_views`]
- [x] [Review][Patch] `attach_parquet_views` rglob 성능 — 250k 파일 규모에서 full walk → init latency 수 초; `next(rglob, None)` early-exit 로 교체 [`parquet_reader.py:attach_parquet_views`]
- [x] [Review][Patch] SIGTERM handler 없음 — systemd kill 시 half-export + JSON result 미출력; `signal.signal(SIGTERM, …)` 로 partial JSON + exit 130 [`scripts/export_parquet_shard.py:main`]
- [x] [Review][Patch] `__NULL__` sentinel 문자열 충돌 + symbol filesystem-hostile char (`/`,`\`,`:`, NUL, 빈 문자열) 미검증 — export 전 `re.match(r'^[A-Za-z0-9_\-]{1,32}$', symbol)` validation [`parquet_shard.py:export_hour_shard`]
- [x] [Review][Patch] SQL f-string 경로·식별자 인젝션 잠재성 — `shard_glob` 의 `'` escape 누락 + `_validate_ident` 일부 경로 미적용 + DuckDB 예약어 수용 [`parquet_reader.py`, `schemas.py:_validate_ident`]
- [x] [Review][Patch] `emit_logger_sync_metric.py` 다수 방어 부족 — `--exit-code` 빈 문자열 crash / orphan `.tmp` 누적 / prev 메트릭 float-parse 실패 / NaN·inf 렌더링 무효 / 동시 lost-update [`scripts/emit_logger_sync_metric.py`]
...
| 2026-04-22 | 1.1.0 | Review-flip — 3-layer adversarial code review (Blind Hunter + Edge Case Hunter + Acceptance Auditor) surfaced 40 findings. Phase A `52ab90d` (CRITICAL 7 + decision): systemd `$EXIT_STATUS` ExecStartPost→ExecStopPost, Prometheus `absent()` guard, `_empty_<table>` TEMP TABLE D1 fix, Parquet atomic tmp+rename, tz-aware enforcement (3 sites) + SET TimeZone='UTC', `_data_equals` null_equal+height prefilter, write-scope AST-based detector, rsync `--delete-after --max-delete=100`. Phase B `9162f07` (MAJOR 17): `parse_hour` validation + SHARD_BAD_HOUR JSON contract, `--tables` up-front validation, DB_LOCKED + SHARD_UNEXPECTED + SHARD_INTERRUPTED contracts, SIGTERM handler, FeatureStore.refresh_views + __init__ lock-leak guard, attach_parquet_views TOCTOU + rglob early-exit + path quote-escape, schemas._validate_ident reserved-word rejection, symbol validation (NULL-sentinel + filesystem-hostile), rsync `--chown` + `--partial-dir` externalisation, timer RandomizedDelaySec + service TimeoutStartSec, installer `install -d/-m 644` atomic, emit NaN/inf sanitisation + orphan tmp sweep (mtime-guarded 300s) + float-parse tolerance. Phase C `7e50557` (MINOR 9 + spec reconciliation): p95 statistics.quantiles, partition pruning tighter assertion, Windows lock-flake gc.collect, CLI finally reopen probe, pytest-xdist anti-pattern removed, DTO/DDL explicit table_name + type pin, Decimal overflow test, duckdb_client docstrings, Dev Agent Record File List `(8)`, spec body reconciliation (AC-5 ExecStopPost literal + Prometheus absent() expr + AC-4 TEMP TABLE rationale). 2 deferred to Story 1.7/1.10 (systemd absolute-path template, symbol-less future table, NTP step-backward, SSH-key boot-time race). 4 dismissed with rationale (gitignore 4-layer negation, published_at_utc TZ round-trip, dry-run env-var vs narrative flag, EXPLAIN vs EXPLAIN ANALYZE relaxation). Tests: 173p → 204p (+31 regressions pinning every CRITICAL/MAJOR/MINOR fix). pre-commit all green. Status: review → done. | Amelia via bmad-code-review + post-review patches |
